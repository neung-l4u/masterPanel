...
function fetchAllItems($url, $headers, $boardId) {
    $allItems = [];
  ...
  boards (ids: ' . $boardId . ') {
    ...
    $groupCursors = []; // groups that need more pagination
    ...
    foreach ($result['data']['boards'][0]['groups'] as $group) {
        ...
        // If this group has a cursor, it has more items
...
$result = fetchAllItems($url, $headers, $boardId);

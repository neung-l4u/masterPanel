<?php
/**
 * getInvoiceByMondayItemTH.php
 *
 * ดึงข้อมูล invoice จาก monday_item_id และ return ในรูปแบบ JSON
 * สำหรับใช้กับ Make.com blueprint ในการส่ง invoice ให้ลูกค้า
 *
 * GET params:
 *   monday_item_id  string  Monday item ID จาก thInvoice.monday_item_id
 */
...
$mondayItemId = trim($_GET['monday_item_id'] ?? $_POST['monday_item_id'] ?? '');

if (empty($mondayItemId) || !preg_match('/^\d+$/', $mondayItemId)) {
    echo json_encode(['success' => false, 'message' => 'ต้องระบุ monday_item_id (ตัวเลข)']);
    exit;
}

// --- ดึงข้อมูล invoice จาก monday_item_id ---
$invRows = $db->query(
...
if (empty($invRows[0])) {
    echo json_encode(['success' => false, 'message' => 'ไม่พบ invoice สำหรับ monday_item_id: ' . $mondayItemId]);
    exit;
}

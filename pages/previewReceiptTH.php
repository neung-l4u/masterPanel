...
require_once dirname(__DIR__) . '/api/invoice/convertToBahtText.php';
...
$receiptRows = $db->query(
    'SELECT `receiptID`, `amount_paid`, `status`, `sentAt`, `createdAt` FROM `thReceipt` WHERE `invoice_id` = ? ORDER BY `id` DESC LIMIT 1',
    $invoiceId
)->fetchAll();
...
// วันที่รับชำระ ใช้วันที่ส่ง receipt จริง ถ้ายังไม่ส่งค่อย fallback ไปวันที่ของรอบบิล
$paidAt = $receipt['sentAt'] ?? null;
$receiptDate = $paidAt
    ? date('d/m/Y', strtotime($paidAt))
    : (!empty($quotation['date']) ? $quotation['date'] : date('d/m/Y', strtotime($row['createdAt'])));

$grandTotal = (float)($summary['grandtotal_inc_vat'] ?? 0);
$subtotal   = (float)($summary['subtotal'] ?? 0);
$vat        = (float)($summary['vat'] ?? 0);
$wht        = (float)($summary['withholdingTax'] ?? 0);
$amountPaid = (float)($receipt['amount_paid'] ?? $summary['net_payment'] ?? $row['amount'] ?? 0);
$thaiText   = convertToBahtText($amountPaid);
...
<head>
    ...
    <style>
        ...
        .invoice-title-section h3 { font-size: 26px; font-weight: normal; margin-top: 10px; }
        .invoice-title-section p { font-size: 18px; color: #666; margin-top: -20px; }
        ...
        .paid-stamp { display: inline-block; margin-top: 6px; padding: 2px 10px; border: 2px solid #16a34a; color: #16a34a; font-weight: 700; border-radius: 4px; font-size: 12px; }
        @media print {
...
<div class="invoice-container">
    ...
    <table class="totals-table">
        <tbody>
            ...
            <?php endif; ?>
            <tr>
                <td class="text-right"><strong>จำนวนเงินที่ได้รับชำระ</strong></td>
                <td class="text-right"><?= fmt($amountPaid) ?> บาท</td>
            </tr>
            <tr>
    ...
    <div class="footer">
        <figure>
            ...
            <ul class="dash">
                ...
                <li>เอกสารฉบับนี้เป็นหลักฐานการรับชำระเงิน ใช้แทนใบกำกับภาษีและใบเสร็จรับเงิน</li>

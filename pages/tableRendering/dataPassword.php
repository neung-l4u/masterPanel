...
if ($teamID==1){ //CS
    $teamCondition = "AND `pwTeam` IN (1,7,0)"; //CS and other
} elseif (($teamID==2) or ($teamID==8) or ($teamID==10) or ($teamID==11)){ //AM > AU USA NZ UK
    $teamCondition = "AND `pwTeam` IN (2,8,10,11,7,0)"; //all AM and other
} elseif ($teamID==3){ //SE
    $teamCondition = "AND `pwTeam` IN (3,7,0)"; //SE and other
} elseif (($teamID==4) or ($teamID==6)){ //HR or CEO
    $teamCondition = "AND `pwTeam` IN (4,6,7,0)"; //all AM and other
} elseif ($teamID==7){ //Other
    $teamCondition = "AND `pwTeam` IN (7,0)"; //Other
} elseif ($teamID==5){ //IT
    $teamCondition = " "; //all team
}
...
foreach ($result as $row) {
    ...
    if ($fullNote == "-"){
        ...
        $row["pwNote"] = "<abbr title='$fullNote'><i class='far fa-eye'></i></abbr>";
    }
    ...
    $typeLabel = "<b>" .$typeLabel.": ". $row["name"] . "</b>";

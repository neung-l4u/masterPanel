<?php
/**
 * Subscription invoice list for the SignUp Logs "Sub Paid" column.
 *
 * The first invoice a customer gets is the signup charge (handled by the
 * First Paid column). Everything after that comes from their subscription -
 * the recurring billing. This returns those, newest first, for the card list.
 */
global $db;
session_start();
...
$cacheDir = __DIR__ . '/../../api/stripe/cache/sub_paid';
...
try {
    ...
    // Stripe returns newest first, so the last one is the signup invoice that
    // the First Paid column already covers.
    ...
    foreach ($all as $inv) {
        ...
        // Sub Paid is about recurring billing: skip the signup invoice, and
        // keep anything that belongs to a subscription.
        if ($inv->id === $firstInvoiceId && empty($subId)) continue;

...
if ($params["action"] == "getMonthly"){
    $result = $db->query('SELECT ROUND(SUM(`value`),2) AS "total" FROM `SucscriptionsMonthly` WHERE `month` = ? AND `year` = ? GROUP BY `month`;', $params["month"], $params["year"])->fetchAll();

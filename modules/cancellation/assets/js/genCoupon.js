...
const genCoupon = () => {
  ...
  const CA = Object.keys(coupons.CA);
  const TH = Object.keys(coupons.TH);
  const UK = Object.keys(coupons.UK);
  const US = Object.keys(coupons.US);
  const AUBox = $(".AU-Coupon");
  const NZBox = $(".NZ-Coupon");
  const CABox = $(".CA-Coupon");
  const THBox = $(".TH-Coupon");
  const UKBox = $(".UK-Coupon");
  const USBox = $(".US-Coupon");

  if(AU.length>0){
      AUBox.empty();
      AU.map( (item) => {
          let link = `<a href="#" onclick="copyToClipboard('${item}');" class="list-group-item list-group-item-action"><i class="fa-regular fa-copy text-success"></i> ${item}</a>`;
          AUBox.append(link)
      });
  }
